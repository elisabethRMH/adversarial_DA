...
for percent_unlabeled in [0.7,0.8, 0.9]:
    for semisup in [True, False]:
        ...
        train_generator_l=train_generator.copy()
        train_generator_l.prune_unlabeled()
        train_generator_l._normalize() #New way of normalizing, after try4
        train_generator_ul = train_generator.copy()
        train_generator_ul.y=train_generator.y_labeled
        train_generator_ul.datalist=train_generator.unlabel_lst
        train_generator_ul.prune=True
        
        training_epoch=40
        training_length=80000
        #percent_unlabeled=0.9
        
        config.out_dir = '/volume1/scratch/ehereman/results_adversarialDA/baselineEOG_e2earnn_1ch_losssum'
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                def dev_step(x_batch, y_batch):
                    ...
                    output_loss, domain_loss, total_loss, yhat, score = sess.run(
                           [arnn.output_loss_mean,arnn.domain_loss_mean, arnn.loss, arnn.predictionC, arnn.score_C], feed_dict)
                    return output_loss, domain_loss, total_loss, yhat, score
                ...
                def evaluate(gen):
                    ...
                    while test_step < num_batch_per_epoch:
                        ...
                        output_loss_, domain_loss_, total_loss_, yhat_, score_ = dev_step(x_batch, y_batch)
...
#                        predictionD=predictionD[0:-1]
                    ...
                    return acc, yhat, score, output_loss, domain_loss, total_loss, ygt
                
                test_acc, test_yhat, test_score, test_output_loss, test_domain_loss, test_total_loss, ygt = evaluate(gen=train_generator_ul)
                print(test_acc)
                savemat(os.path.join(config.out_path, "train_retF34.mat"), dict(yhat = test_yhat, acc = test_acc, score = test_score,
                                                                         output_loss = test_output_loss,
                                                                     total_loss = test_total_loss, ygt=ygt))                
        
                test_acc, test_yhat, test_score, test_output_loss, test_domain_loss, test_total_loss, ygt = evaluate(gen=train_generator_l)
                print(test_acc)
                savemat(os.path.join(config.out_path, "train_retC34.mat"), dict(yhat = test_yhat, acc = test_acc, score = test_score,
                                                                         output_loss = test_output_loss,
                                                                     total_loss = test_total_loss, ygt=ygt))
        
#                test_acc, test_yhat, test_score, test_output_loss, test_domain_loss, test_total_loss, ygt = evaluate(gen=test_generator2)
#                print(test_acc)
#                savemat(os.path.join(config.out_path, "test_retF34.mat"), dict(yhat = test_yhat, acc = test_acc, score = test_score,
#                                                                         output_loss = test_output_loss,
#                                                                     total_loss = test_total_loss, ygt=ygt))                
#        
#                test_acc, test_yhat, test_score, test_output_loss, test_domain_loss, test_total_loss, ygt = evaluate(gen=test_generator1)
#                print(test_acc)
#                savemat(os.path.join(config.out_path, "test_retC34.mat"), dict(yhat = test_yhat, acc = test_acc, score = test_score,
#                                                                         output_loss = test_output_loss,
#                                                                     total_loss = test_total_loss, ygt=ygt))
#                test_acc, test_yhat, test_score, test_output_loss, test_domain_loss, test_total_loss, ygt = evaluate(gen=eval_generator4)
#                print(test_acc)
#                savemat(os.path.join(config.out_path, "eval_retF34.mat"), dict(yhat = test_yhat, acc = test_acc, score = test_score,
#                                                                         output_loss = test_output_loss,
#                                                                     total_loss = test_total_loss, ygt=ygt))                                
#                test_acc, test_yhat, test_score, test_output_loss, test_domain_loss, test_total_loss, ygt = evaluate(gen=eval_generator3)
#                print(test_acc)
