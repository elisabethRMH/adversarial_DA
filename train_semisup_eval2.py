...
config.out_dir = '/volume1/scratch/ehereman/results_adversarialDA/V2_C34vsEOGtry19_classifierinput'
...
config.domain_lambda = 0.005 #used to be 0.002
#config.l2_reg_lambda = 0.01 #EXP try 5
# path where checkpoint models are stored
#config.batch_size = 100
config.domainclassifierstage2=False
config.add_classifieroutput=True
...
def run(percent_unlabeled, normalize=True):
    ...
    tmp=sorted(train_generator.datalist)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            model = AdversarialNetwork(config, session=sess)
            model.initialize()
            model.train( train_generator, eval_generator, test_generator, training_epoch*2*4, training_length/2)
...
#    tmp2=run(0.97)
    ...
    config.out_dir = '/volume1/scratch/ehereman/results_adversarialDA/V2_C34vsEOGtry19_classifierinput_2'
    tmpa=run(0.8)
    tmpa2=run(0.9)
    tmpa3=run(0.7)
#    tmpa=run(0.95)
...
#    config.out_dir = '/volume1/scratch/ehereman/results_adversarialDA/V2_C34vsF34try8_newNorm2_L2reg_minlayer_equalbatches_3'
    config.out_dir = '/volume1/scratch/ehereman/results_adversarialDA/V2_C34vsEOGtry19_classifierinput_3'
    tmpb=run(0.8)
    tmpb2=run(0.9)
    tmpb3=run(0.7)
#    tmpb=run(0.95)
