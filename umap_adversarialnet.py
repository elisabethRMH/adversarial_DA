...
# My Parameters
percent_unlabeled=0.7
#tf.app.flags.DEFINE_string("out_dir", '/volume1/scratch/ehereman/results_adversarialDA/baseline_e2earnn_1ch_losssum/', "Point to output directory")
tf.app.flags.DEFINE_string("out_dir", '/volume1/scratch/ehereman/results_adversarialDA/V2_C34vsEOGtry17_newNorm2_L2reg_minlayer_equalbatches', "Point to output directory")
...
train_generator2=train_generator.copy()
eval_generator2=eval_generator.copy()
test_generator2=test_generator.copy()
train_generator2.switch_channels(channel_to_use=channel_to_use)
eval_generator2.switch_channels(channel_to_use=channel_to_use)
test_generator2.switch_channels(channel_to_use=channel_to_use)
    ...
    with sess.as_default():
        ...
        def evaluate(gen):
            ...
            while test_step < num_batch_per_epoch:
                ...
                y_batch=y_batch[:,0]
                output_loss_, total_loss_, yhat_, score_, features_ = dev_step(x_batch, y_batch) #E: score is raw output without applying softmax?
                feat[(test_step)*config.batch_size : (test_step+1)*config.batch_size, :] = features_
            ...
            if len(gen.datalist) - test_step*config.batch_size==1:
                ...
                x_batch=x_batch[:,0,:,:,0:3]
                y_batch=y_batch[:,0]
                output_loss_, total_loss_, yhat_, score_, features_ = dev_step(x_batch, y_batch)
                ygt[ (test_step)*config.batch_size : len(gen.datalist)] = y_batch
                feat[test_step*config.batch_size:len(gen.datalist)] = features_
                score[(test_step)*config.batch_size :len(gen.datalist),:] = score_
            ygt+=1
            return ygt, feat, score
        

        ygt, feat,score = evaluate(gen=test_generator)
        ygt2, feat2, score2 = evaluate(gen=train_generator)
        ygt3, feat3, score3 = evaluate(gen=eval_generator)
        ygtf, featf, scoref = evaluate(gen=test_generator2)
        ygtf2, featf2, scoref2 = evaluate(gen=train_generator2)
        ygtf3, featf3, scoref3 = evaluate(gen=eval_generator2)
        ygt_c= np.concatenate([ygt, ygt2, ygt3])
        ygt_t= np.concatenate([ygt, ygt3])
        score_c = np.concatenate([score,score2,score3])
        yhat_c= np.argmax(score_c, axis=1)+1
        feat_c= np.concatenate([feat, feat2, feat3])
        feat_cf = np.concatenate([featf,featf2,featf3])
        score_cf=np.concatenate([scoref, scoref2, scoref3])
        yhat_cf= np.argmax(score_cf, axis=1)+1
...
#        dsc_t = np.concatenate([dsc, dsc3])
        ...
        dff_score=np.mean(np.abs(score_c-score_cf))
        yhat_c=np.argmax(score_c,axis=1)
        yhat_cf = np.argmax(score_cf, axis=1)
        dff_yhat=np.mean(np.abs(yhat_c-yhat_cf))
        print(dff_feat, dff_score, dff_yhat)
