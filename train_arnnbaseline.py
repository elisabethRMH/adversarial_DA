...
for num_patients in [1,2,5]:
    fold=0
    print('Fold: ', fold)
    train_files=files_folds['train_sub'][:,0:num_patients]#[fold][0][0]
    eval_files=files_folds['eval_sub']#[fold][0][0]
    test_files=files_folds['test_sub']#[fold][0][0]
    
    
    training_epoch=85
    training_length=100000*num_patients/40*4
    #percent_unlabeled=0.9
    
    config= Config()
    config.out_dir = '/volume1/scratch/ehereman/results_adversarialDA/baselineEOG_e2earnn_1ch_losssum_only{:d}pat'.format(num_patients)
